...
def is_python_or_html_file(file):
    """
    ...
    if os.path.isfile(path):
        ...
        for root, dirs, files in os.walk(path):
            ...
            for file in files:
                ...
                if is_python_or_html_file(filepath):
...
def inizialize_data(filepath):
    ...
    """
    data = {"total_lines": 0, "files": {}, "directory": {}}
...
def check_valid_args(paths):
    ...
    """
    return all((is_python_or_html_file(path) or os.path.isdir(path) for path in paths))
